...
- M7: Sequencing is `wire::send(t, payload, drop_counter)`: one `TX_LOCK` section takes `seq`, encodes the `FrameBuf` and `try_send`s it. A frame that fails to encode also counts as a drop, so it too leaves a seq gap.
- M7: `console-text` `tx_task` decodes each queued `FrameBuf` and prints one line per frame; the binary build writes it with `embedded_io_async::Write::write_all` (esp-hal implements the 0.7 trait under `unstable`).
- M7: `FrameLogger` sends `record.args()` only (no target), cut at a char boundary to 200 B. It is installed right after `esp_rtos::start`; `main` logs `radio up ch=6` after the boot channel is set.
- M7: HELLO `git_sha` is `option_env!("RID_GIT_SHA")` truncated to 8 B, else `unknown`. `main` returns after spawning tasks and queuing the boot HELLO; the M6 counter line is gone from `console-text` (STATUS lines replace it).
- M7 SERIAL CHECK passed 2026-09-24 (agent, COM3, native Windows one-byte reads): `espflash flash --non-interactive` then immediately `rid-host live --port COM3 --stats-every 60` under `timeout 610`. Result: HELLO seq 1 at 0.3 s, then every 30.0 s (21 total); STATUS every 1.000 s (±43 µs); `bad_crc=0 seq_gaps=0 seq_reorder=0`; all drop counters 0; heap_free 55960 throughout. One `text_chunks` = the ROM bootloader log, which carries no trailing `0x00` and therefore swallowed LOG seq 0; expected whenever the host is attached across a reset.
